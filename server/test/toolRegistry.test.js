...
test('tool registry exposes the same native search and visual tools in tutor mode', () => {
  ...
  assert.deepEqual(chat.definitions.map((tool) => tool.function.name), ['code_interpreter', 'workspace_agent', 'render_visualization', 'web_search', 'web_fetch', 'create_plan', 'create_spec', 'arxiv_search']);
  const tutor = createToolRegistry({ codeInterpreterToolDef: null, mode: 'tutor' });
  assert.deepEqual(tutor.definitions.map((tool) => tool.function.name), ['workspace_agent', 'render_visualization', 'web_search', 'web_fetch', 'create_plan', 'create_spec', 'arxiv_search']);
