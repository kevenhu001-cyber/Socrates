...
test('Every phase-1 ready entry names an ocService', () => {
...
test('buildOpenConnectorStubCatalog covers the full ready inventory as stubs', () => {
  ...
  const stubs = buildOpenConnectorStubCatalog();
...
test('Jimeng chat tool execution fails closed when the OOMOL gateway is not configured', async () => {
  /* Cloud-only execution: a connected row reaches getProjectConnector(),
     which is null without OOMOL_PROJECT_API_KEY. A configured-key run needs
     a live gateway, so the deterministic unit assertion is the not-configured
     envelope. */
  const previousKey = process.env.OOMOL_PROJECT_API_KEY;
  delete process.env.OOMOL_PROJECT_API_KEY;
  try {
    const connected = { status: 'connected', connectionName: 'socrates', connectedAccountId: 'ca-1' };
    ...
    assert.equal(submitted.errorCode, 'project_connector_not_configured');
    assert.equal(submitted.userMessage, 'The connector service is not configured yet.');

    const result = await executeOpenConnectorTool(
    ...
    assert.equal(result.status, 'failed');
    assert.equal(result.errorCode, 'project_connector_not_configured');
  } finally {
    if (previousKey === undefined) delete process.env.OOMOL_PROJECT_API_KEY;
    else process.env.OOMOL_PROJECT_API_KEY = previousKey;
  }
});
...
test('Cloud auth snapshot covers every ready service with usable auth', () => {
  ...
  for (const service of readyServices) {
    ...
    assert.ok(meta, `${service} missing from cloud snapshot (maintain server/src/services/openConnectorCloudAuth.generated.ts)`);
