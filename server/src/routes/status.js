...
let feedCache = { at: 0, promise: null };
...
async function uptimeFor(component, startMs, endMs) {
  const db = getDb();
...
async function computeServiceAvailability(sinceMs, nowMs) {
  const db = getDb();
  const rows = await db.select({
...
async function buildTimeline(sinceMs, nowMs) {
  const db = getDb();
...
async function buildIncidents(sinceMs, nowMs) {
  const db = getDb();
  const rows = await db.select({
...
router.get('/confirm', async (req, res, next) => {
  try {
    ...
    const [sub] = await db.select().from(statusSubscribers)
      .where(and(eq(statusSubscribers.token, token), sql`${statusSubscribers.confirmedAt} IS NULL`))
      .limit(1);
