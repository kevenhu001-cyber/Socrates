...
router.get('/limits', async (req, res, next) => {
  try {
    const { tier } = req.user || {};
    const limits: Record<string, { tokenQuota: number | null; hardLimit: boolean }> = {
...
router.get('/daily', async (req, res, next) => {
  try {
    ...
    const since = new Date(Date.now() - parseInt(days as string, 10) * 86400000);
    ...
    return res.json({ days: parseInt(days as string, 10), entries: rows });
  } catch (err) { next(err); }
});
