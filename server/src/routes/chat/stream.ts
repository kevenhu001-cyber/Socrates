...
export function registerStreamRoute(router: Router) {
  ...
  router.post('/stream', requireAuth, async (req, res, next) => {
    try {
      ...
      let connectorConnectionsByProvider: Record<string, typeof connectorConnections.$inferSelect | undefined> = {};
      /** @type {Record<string, import('../../db/schema.js').projectConnectorConnections.$inferSelect | undefined>} */
      let projectConnectorConnectionsByProvider: Record<string, typeof projectConnectorConnections.$inferSelect | undefined> = {};
      if (req.userId) {
        try {
          const db = getDb();
...
`);
              ...
              const toolResult: ToolResult = await executeConnectorTool(toolName, args, connection);
