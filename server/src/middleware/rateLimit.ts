...
const CHAT_TIER_MULTIPLIERS: Record<string, number> = {
  diophantus: 1,                     // 240/h
  riemann:    2,                     // 480/h
  descartes:  4,                     // 960/h
  euclid:     8,                     // 1920/h
};
function chatLimitForTier(tier?: string | null): number {
...
function tutorLimitForTier(tier?: string | null): number {
  const mult = CHAT_TIER_MULTIPLIERS[tier ?? ''] || 1;
...
function chatRateLimitHandler(req: Request & { rateLimit?: { resetTime?: Date } }, res: Response) {
...
export function pickChatLimiterFor(req: Request) {
