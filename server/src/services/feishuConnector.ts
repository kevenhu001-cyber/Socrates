...
export function feishuAuthorizationUrl(state: string) {
...
async function feishuJson(url: string, options: RequestInit): Promise<Record<string, unknown>> {
...
function tokenFields(token: FeishuTokenResponse) {
...
export async function completeFeishuAuthorization(code: string) {
  ...
  const user = profile.data || ({} as FeishuUser);
  return {
...
export async function refreshFeishuAuthorization(connection: { refreshTokenCiphertext: string | null }) {
