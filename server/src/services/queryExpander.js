 ...
 */
export async function expandQuery(query, opts = {}) {
  ...
  let cfg;
  try {
    cfg = await getActiveApiKey(opts.userId);
  } catch (e) {
    return [q];
  }
  ...
  try {
    ...
    if (e && e.name !== 'AbortError' && e.message && e.message.indexOf('silence-timeout') < 0) {
      console.warn('[queryExpander] LLM call failed:', e.message || e);
    }
    return [q];
  }
}
