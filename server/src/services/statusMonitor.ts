...
function pubsubUp() {
  try {
    ...
    return (s as { pgListenerReady?: boolean })?.pgListenerReady === true;
  } catch {
    return false;
  }
}
...
async function probe(component: string) {
  const db = await dbUp();
  switch (component) {
    case 'Database':
    case 'Authentication':
...
async function recordTransition(component: string, from: string | null, to: string, detail: unknown) {
...
let metricsSamples: { t: number; ms: number; ok: boolean }[] = []; // { t, ms, ok }

export function recordRequestSample(durationMs: number, ok: boolean) {
  const now = Date.now();
...
async function tick() {
  for (const component of COMPONENTS) {
    let result: { state: string; detail?: unknown };
    try {
      result = await probe(component);
    } catch (err) {
      result = { state: 'down', detail: { reason: String((err as Error)?.message || err) } };
    }
...
async function notifySubscribers(component: string, from: string, to: string) {
  var db = getDb();
...
export function getLiveState(component: string) {
...
export async function startStatusMonitor() {
  try {
    ...
    console.warn('[status-monitor] seed skipped:', (err as Error).message);
  }
