...
export async function completeGithubAuthorization(code) {
  ...
  const [profile, installations] = await Promise.all([
    githubJson(`${GITHUB_API}/user`, { headers: githubHeaders(token.access_token) }),
    githubJson(`${GITHUB_API}/user/installations`, { headers: githubHeaders(token.access_token) }),
  ]);
  return {
...
export function verifyGithubWebhook(rawBody, signature) {
...
export async function listGithubInstallationRepositories(connection) {
  const ids = Array.isArray(connection.installationIds) ? connection.installationIds : [];
  const installationId = ids.find((value) => Number.isSafeInteger(Number(value)));
  if (!installationId) return [];
  const jwt = await createGithubAppJwt();
  const installationToken = await githubJson(`${GITHUB_API}/app/installations/${installationId}/access_tokens`, {
  ...
  const repositories = await githubJson(`${GITHUB_API}/installation/repositories?per_page=100`, {
