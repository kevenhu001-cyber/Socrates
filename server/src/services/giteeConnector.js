...
export function giteeAuthorizationUrl(state) {
...
function tokenRequest(payload) {
  return giteeJson(`${GITEE}/oauth/token`, {
    method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', Accept: 'application/json' }, body: new URLSearchParams(payload),
  });
}

export async function completeGiteeAuthorization(code) {
  ...
  const profile = await giteeJson(`${GITEE}/api/v5/user?${new URLSearchParams({ access_token: token.access_token })}`);
  return {
...
export async function refreshGiteeAuthorization(connection) {
...
export async function listGiteeRepositories(connection) {
  ...
  const repositories = await giteeJson(`${GITEE}/api/v5/user/repos?${query}`);
