...
const MOBILE_WEB_SESSION_RE = /^mw\.(projects|scheduled|plugins|knowledge|mistakes|skills|api-settings|profile|usage|storage)\.([a-f0-9]{64})$/;
...
export async function register(email: string, password: string) {
  ...
  if (existing) {
    ...
    try { await sendDuplicateRegistrationEmail(normalizedEmail); } catch (e) { console.warn('[auth] duplicate-registration notice failed:', (e as Error).message); }
    return { ok: true };
  }
 ...
 */
export async function logout(sid: string | null | undefined) {
  if (!sid) return;
  const db = getDb();
 ...
 */
export async function getResetInfo(token: string) {
 ...
 */
export async function changePassword(userId: string, oldPassword: string, newPassword: string, currentSid?: string | null) {
 ...
 */
export async function deleteAccount(userId: string, email?: string | null) {
  const db = getDb();
  ...
  try {
    ...
    for (const r of rows) {
      ...
      catch (e) { if ((e as NodeJS.ErrnoException).code !== 'ENOENT') console.warn('[auth] delete-account: unlink failed for', resolved, (e as Error).message); }
    }
  } catch (e) {
    ...
    console.warn('[auth] delete-account: file sweep failed:', (e as Error).message);
  }
