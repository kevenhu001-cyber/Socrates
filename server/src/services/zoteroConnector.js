...
async function zoteroJson(path, apiKey) {
  let response;
  try {
...
export async function createZoteroConnection(apiKey) {
  if (!validApiKey(apiKey)) {
    throw new ZoteroConnectorError('ZOTERO_INVALID_KEY', 'Enter a valid Zotero API Key.');
  }
  const keyInfo = await zoteroJson('/keys/current', apiKey);
  const userId = String(keyInfo.userID || '');
  const userAccess = keyInfo.access?.user || {};
...
export async function listZoteroItems(connection, query = '') {
