...
declare global {
  interface Window {
    ...
    getCsrfToken: () => string;
    makeAIWatchdog: () => any;
    sleepBackoff: (attempt: number, retryAfter?: string | null) => Promise<void>;
    ...
    STREAM_RETRYABLE_STATUS: Record<number, boolean>;
    STREAM_MAX_ATTEMPTS: number;
    isMiniMaxProvider?: () => boolean;
    getReasoningEffort?: () => string;
    _activeChatAbort: ((reason?: string) => void) & { _fromThisCall?: boolean };
  }
}

/* Callback function types */
type OnDeltaCallback = (text: string, full: string) => void;
type OnThinkingCallback = (text: string) => void;

interface StreamOptions {
  onToolUse?: (data: any) => void;
  onToolResult?: (data: any) => void;
  onToolProgress?: (data: any) => void;
  onExecutionStart?: (data: any) => void;
...
export async function callAPIStream(
  ...
  while (attempt < STREAM_MAX_ATTEMPTS) {
    ...
    window._activeChatAbort = function (reason?: string) { try { ac.abort(reason); } catch (_) { } } as typeof window._activeChatAbort;
    window._activeChatAbort._fromThisCall = true;
    var tmo: ReturnType<typeof setTimeout> | null = setTimeout(function () { try { ac.abort("timeout"); } catch (_) { } }, STREAM_TIMEOUT_MS);
    var hbTmo: ReturnType<typeof setTimeout> | null = null;
    ...
    try {
      ...
      var apiBody: Record<string, any> = { messages: messages, temperature: 0.7, max_tokens: maxTokens };
      ...
      if (eStatus === 429) {
        ...
        var retryAfterSec429: number | null = null;
    ...
    var reader: ReadableStreamDefaultReader<Uint8Array> | null = resp.body.getReader();
    var decoder = new TextDecoder("utf-8");
    var buf = "";
    var full = "";
    var formattedHtml: Record<string, any> | null = null;
          ...
          try {
            ...
            if (typeof delta === "string" && delta.length > 0) {
              if (!thinkOpen) {
                ...
                var openIdx = probe2.indexOf("<think>");
                if (openIdx === -1) {
                  ...
                  var safeLen = Math.max(0, probe2.length - 7);
                  var safeStr = probe2.slice(0, safeLen);
                  thinkTail = probe2.slice(safeLen);
                  if (safeStr.length > 0) {
                    full += safeStr;
                    try { if (onDelta) { onDelta(safeStr, full); } } catch (deltaErr) {
                      console.warn("[API stream] onDelta threw:", deltaErr && (deltaErr as Error).message);
                    }
                  }
                } else {
                  ...
                  var before = probe2.slice(0, openIdx);
                  var afterOpen = probe2.slice(openIdx + "<think>".length);
                  thinkTail = "";
                  if (before.length > 0) {
                    full += before;
              ...
              if (thinkOpen) {
                /* Inside a think block. Scan for the closing tag,
                   ...
                   tags. */
                var probe3 = thinkTail + delta;
                var closeIdx = probe3.indexOf("</think>");
                while (closeIdx !== -1) {
                  var inside = probe3.slice(0, closeIdx);
                  thinkBuf += inside;
                  ...
                  var after = probe3.slice(closeIdx + "</think>".length);
                  ...
                  if (thinkTail.indexOf("<think>") !== -1 || bodyStr.indexOf("<think>") !== -1) {
                    ...
                    if (remaining.length > 0) {
                      ...
                      if (oi2 === -1) {
                        ...
                        var bf2 = remaining.slice(0, oi2);
                        var ao2 = remaining.slice(oi2 + "<think>".length);
                        if (bf2.length > 0) {
                          full += bf2;
                          try { if (onDelta) { onDelta(bf2, full); } } catch (deltaErr) {
                            console.warn("[API stream] onDelta threw:", deltaErr && (deltaErr as Error).message);
                          }
                        }
                        thinkOpen = true;
                        thinkBuf = "";
                        delta = ao2;
                        probe3 = thinkTail + delta;
                        closeIdx = probe3.indexOf("</think>");
                        continue;
                      }
                    }
                  }
                ...
                if (thinkOpen) {
                  ...
                  thinkBuf += probe3.slice(0, Math.max(0, probe3.length - 7));
                  thinkTail = probe3.slice(Math.max(0, probe3.length - 7));
    ...
    if (window._activeChatAbort && window._activeChatAbort._fromThisCall) {
      window._activeChatAbort = null as any;
    }
...
/* ── Type-only export for the showToast global used in this module ── */
declare var showToast: ((msg: string, duration?: number) => void) | undefined;
