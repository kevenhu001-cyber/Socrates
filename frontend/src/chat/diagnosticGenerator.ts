...
export async function generateDiagnosticQuestions(
  topic: string,
  language: string,
  onProgress?: ((current: number, total: number, q: DiagQuestion | null) => void) | null,
  shouldCancel?: (() => boolean) | null,
): Promise<DiagQuestion[] | null> {
  const langNames: Record<string, string> = { zh: 'Chinese', ja: 'Japanese', ko: 'Korean', ru: 'Russian', ar: 'Arabic', en: 'English' };
  const langName = langNames[language] || 'English';
  const all: DiagQuestion[] = [];
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const aspect = DIAG_ASPECTS[i] || DIAG_ASPECTS[DIAG_ASPECTS.length - 1];
    const prevBlock = previousTexts.length
    ...
    const resp = await callAPI(msgs, MAX_TOKENS_DIAG, i === 0 ? DIAG_FIRST_TIMEOUT_MS : undefined);
    if (!resp) {
      const state = getState();
      if (!state.lastCallError) state.lastCallError = 'Diag call returned empty response';
      break;
    }
    const q = parseOneDiagResponse(resp, i);
    if (!q) {
      break;
    }
    all.push(q);
  ...
  /* If we got fewer than 3 of 5 questions, treat the whole call as
     failed and let the caller fall back to mock. */
