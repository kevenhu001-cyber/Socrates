// chat/webLinks.ts — URL extraction and page-fetch helpers.
// Extracted from main.js region 22. Depends on util/api.js for the
// batch-fetch endpoint call.
...
export function looksLikeUserMentionedSite(text: string): boolean {
...
export function extractHttpUrls(text: string): string[] {
  ...
  const FILE_EXTS = ("pdf doc docx xls xlsx ppt pptx zip rar 7z tar gz " +
    "jpg jpeg png gif webp svg mp3 mp4 mov avi mkv exe dmg iso " +
    "txt md rtf csv json xml html htm").split(" ");
  const re2 = /(?:^|[^\w一-鿿＠@])([a-z0-9](?:[a-z0-9-]*[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)+(?:\/[^\s一-鿿　-〿＀-￯"'<>)\]】」』]*)?)/gi;
  while ((m = re2.exec(text)) !== null) {
    ...
    const hasPath = d.indexOf("/") !== -1;
    const hasWww = /^www\./i.test(d);
    if (!hasPath && !hasWww) {
      const t = tld.toLowerCase();
      let isFile = false;
      for (let fi = 0; fi < FILE_EXTS.length; fi++) { if (FILE_EXTS[fi] === t) { isFile = true; break } }
      if (isFile) continue;
    }
    add("https://" + d);
    if (out.length >= 3) break;
...
/* Fetch up to 3 pages in parallel and format them as [Referenced page]
   ...
   in its answer. */
export async function fetchPagesForContext(urls: string[]): Promise<{ blocks: string[]; results: FetchBatchResult[] }> {
  if (!Array.isArray(urls) || !urls.length) return { blocks: [], results: [] };
  try {
    const r = await apiFetch("/api/fetch-batch", { method: "POST", body: JSON.stringify({ urls: urls }) }) as { results?: FetchBatchResult[] };
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      if (f && f.ok && f.content) {
        ...
        const reason = (f && f.reason) || "fetch failed";
