...
export function scheduleActiveTurnToTop(
  ...
  /* P_turn-reserve-follow — LobeHub recomputes its trailing spacer whenever
     the viewport changes (keyboard open/close, window resize, composer
     growth); the prompt then keeps its top offset. A send-time reserve is a
     single measurement, so without this the prompt slides by the viewport
     delta the moment the keyboard closes. Keep the active turn's reserve
     matched to the live transcript height, and re-align a pinned reader
     after the layout commits. The observer retires once the answer outgrows
     the reserve or the reserve is cleared; "the spacer is gone" is the same
     end state as LobeHub collapsing its spacer to zero. */
  let reserveObserver: ResizeObserver | null = null;
  function disconnectReserveObserver(): void {
    if (reserveObserver) {
      try { reserveObserver.disconnect(); } catch (_) { /* already detached */ }
      reserveObserver = null;
    }
  }
  function currentAnchorEntry(): MessageEntry | null {
    const stored = stateStore.read('messages') as MessageEntry[];
    for (let i = stored.length - 1; i >= 0; i -= 1) {
  ...
  function refreshTurnReserve(): void {
    if (!list || retryViewport) return;
    const mounted = row() as HTMLElement | null;
    if (!mounted || mounted.isConnected === false) {
      disconnectReserveObserver();
      return;
    }
    const entry = currentAnchorEntry();
    ...
    /* The answer has outgrown the reserve: the row is content-sized now and
       sticky-bottom owns the scroll; matching the viewport has no layout
       left to act on. */
    if (mounted.getBoundingClientRect().height > reserve + 1) {
      disconnectReserveObserver();
      return;
    }
    const users = list.querySelectorAll('.msg.user');
    const prompt = users.length ? (users[users.length - 1] as HTMLElement) : null;
    if (!prompt) return;
    const styles = getComputedStyle(list);
    const bottomPadding = parseFloat(styles.paddingBottom) || 0;
    const nextReserve = Math.max(
      120,
      Math.round(list.clientHeight - prompt.getBoundingClientRect().height - bottomPadding - 24),
    );
    if (Math.abs(nextReserve - reserve) < 4) return;
    const updated = updateMessageSnapshot(entry, {
      _turnAnchorMinHeight: nextReserve,
      _turnAnchorMode: entry._turnAnchorMode || 'turn',
      _turnViewportTarget: entry._turnViewportTarget ?? 12,
      _toolRunRev: (entry._toolRunRev || 0) + 1,
    }, true) as MessageEntry | null;
    publishReactChatRuntime({ type: 'tool-run-updated', messageId: String(clientId || '') });
    /* A history reader owns their position and the reserve sits below the
       prompt, so nothing above it moves. A pinned reader needs the prompt
       re-aligned after the React commit changes the layout. */
    if (stateStore.read('_userScrolledAway')) return;
    const targetOffset = Number(updated?._turnViewportTarget ?? entry._turnViewportTarget);
    const offset = Number.isFinite(targetOffset) ? targetOffset : 12;
    requestAnimationFrame(function settleReserveFollow() {
      if (!list || !list.isConnected) return;
      const rows = list.querySelectorAll('.msg.user');
      const anchor = rows.length ? (rows[rows.length - 1] as HTMLElement) : null;
      if (!anchor) return;
      const actual = anchor.getBoundingClientRect().top - list.getBoundingClientRect().top;
  ...
  if (typeof ResizeObserver === 'function' && list) {
    ...
    reserveObserver = new ResizeObserver(function () {
      refreshTurnReserve();
    });
    try { reserveObserver.observe(observedList, { box: 'border-box' }); }
    catch (_) {
      try { reserveObserver.observe(observedList); } catch (_) { /* detached list */ }
    }
  }
