...
function getState(): Record<string, any> { return (window as any).state; }
function tr(key: string): string { return typeof (window as any).t === 'function' ? (window as any).tr(key) : key; }
...
interface FetchWebContextResult {
  ok: boolean;
  reason: string;
  results: number;
  context: string;
...
interface WebSearchWithRetryOpts {
  signal?: AbortSignal;
  onStep?: (ev: { kind: string; data: any }) => void;
  ceilingMs?: number;
}

export async function fetchWebContext(topic: string, opts?: FetchWebContextOpts): Promise<FetchWebContextResult> {
  opts = opts || {};
  ...
  function _emit(kind: string, data?: any): void {
  ...
  if (opts.background) {
    ...
    (window as any).__bgPillTimerClear = _bgClearPillTimer;
  } else {
  ...
  if (offlineGuard()) {
    if (opts.background) { try { (window as any).__bgPillTimerClear && (window as any).__bgPillTimerClear(); } catch (_) { } }
  ...
  try {
    ...
    var searches = queries.map(function (q: string, idx: number) {
      _emit("querying", { query: q, idx: idx, total: queries.length });
      _wsRemaining--;
      return apiFetchRaw("/api/web-search", {
        method: "POST",
        body: { query: q, count: 8 } as any,
        signal: ac.signal
      }).then(function (r: Response) {
        return r.json().then(function (d: any) { return { ok: true, query: q, results: (d && d.results) || [] }; });
      }).catch(function (e: any) { return { ok: false, status: e && e.status, reason: e && e.message, results: [] }; });
    }) as Promise<{ ok: boolean; query: string; results: any[]; status?: number; reason?: string }>[];
    ...
    if (!searchResults.length && queries[0] !== topic && _wsRemaining > 0) {
      ...
      try {
        var r2 = await apiFetchRaw("/api/web-search", {
          method: "POST",
          body: { query: topic, count: 8 } as any,
          signal: ac.signal
        });
        var d2 = await r2.json();
        searchResults = (d2.results || []).map(function (x: any) { return Object.assign({ matchedQuery: topic }, x); });
    ...
    var topUrls = d.results.slice(0, 8).map(function (x: WebSearchResult) { return x.url; });
    var fetched: Array<{ url?: string; ok?: boolean; content?: string; truncated?: boolean }> = [];
    if (topUrls.length) {
      _emit("fetching", { urls: topUrls, count: topUrls.length });
      try {
        var fb = await apiFetchRaw("/api/fetch-batch", {
          method: "POST",
          body: { urls: topUrls } as any,
          signal: ac.signal
        });
    ...
    var byUrl: Record<string, any> = {};
    fetched.forEach(function (f) { if (f && f.url) byUrl[f.url] = f; });
    var enriched: WebSearchResult[] = d.results.map(function (x: WebSearchResult, i: number) {
      var f = byUrl[x.url!];
    ...
    var lines = enriched.map(function (x: WebSearchResult, i: number) {
    ...
    /* Phase 3: emit per-source engine breakdown so the UI can render
       "Wikipedia \xd72, arXiv \xd71, Bing \xd73" in the summary. */
    {
      var engineCounts: Record<string, number> = {};
    ...
    try { setSearchPill("ok", enriched.length, enriched.length + " sources" + (fetchedCount ? " \xb7 " + fetchedCount + " full" : "")); } catch (_) { }
    return { ok: true, reason: "ok", results: enriched.length, context: ctx, sources: enriched };
  } catch (e: any) {
    clearTimeout(tmo);
    emsg = (e && e.message) || String(e);
...
function _buildJudgeDigest(sources: WebSearchResult[]): string {
  if (!Array.isArray(sources)) return "[]";
  var top = (sources.slice(0, 5)).map(function (s: WebSearchResult) {
    return {
...
async function judgeSearchQuality(sources: WebSearchResult[], topic: string, opts?: any): Promise<JudgeVerdict> {
  opts = opts || {};
  ...
  try {
    var raw = await callAPI(msgs as any, 80);
    clearTimeout(timer);
    ...
    if (typeof raw === "string") text = raw;
      ...
      var rawObj = raw as any;
      if (typeof rawObj.content === "string") text = rawObj.content;
      else if (typeof rawObj.text === "string") text = rawObj.text;
      else if (Array.isArray(rawObj.choices) && rawObj.choices[0] && rawObj.choices[0].message) {
        text = String(rawObj.choices[0].message.content || "");
      }
    }
    text = (text || "").trim();
 ...
 */
export async function webSearchWithRetry(topic: string, opts?: WebSearchWithRetryOpts): Promise<FetchWebContextResult> {
  opts = opts || {};
...
export function extractChatQuery(): string {
...
var _rewriterCache = new Map<string, string[]>();
...
export async function rewriteQueryForSearch(rawText: string): Promise<string[] | null> {
  ...
  try {
    var r = await apiFetch("/api/chat", { method: "POST", body: { messages: msgs, temperature: 0.3, max_tokens: 250 } as any, signal: ac.signal });
    clearTimeout(tmo);
    var rAny = r as any;
    if (!rAny || !rAny.choices || !rAny.choices[0] || !rAny.choices[0].message) return null;
    var txt = rAny.choices[0].message.content || "";
    ...
    var cleaned = arr.filter(function (x: any) { return typeof x === "string" && x.trim(); }).map(function (x: string) { return x.trim(); });
...
export function shouldRefreshSearch(): boolean {
...
export function setSearchPill(kind: string, count: number, label: string): void {
  var p = document.getElementById("searchPill");
  if (!p) return;
  if (!webSearchOn) { p.classList.add("hidden"); return; }
  p.classList.remove("hidden");
  p.className = "search-pill " + kind;
  p.textContent = tr("chat.webSearchLabel") + " " + (label || (count > 0 ? tr("chat.webSearchSources").replace("{n}", String(count)) : ""));
  ...
  /* Background refresh installs a watchdog timer that flips the pill
     ...
     timeout error. */
  try { if (typeof (window as any).__bgPillTimerClear === "function") { (window as any).__bgPillTimerClear(); (window as any).__bgPillTimerClear = null; } } catch (_) { }
}
