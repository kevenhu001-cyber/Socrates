// chat/mocks.ts — Wave 0c of main-js-split plan.
...
function _origGenerateSocraticQuestion(node: SocraticNode, domain: string): QuestionResult {
  ...
  const pool = qs[node.status] || qs.fuzzy;
  const idx = Math.floor(Math.random() * pool.length);
  return { text: pool[idx], node: node };
}

function _origGenerateFollowUp(answer: string, node: SocraticNode, domain: string): string {
  const phrase = extractKeyPhrase(answer);
  const fus: string[] = [
...
function extractKeyPhrase(text: string): string {
  ...
  const start = Math.floor(Math.random() * Math.min(words.length - 3, words.length));
  return words.slice(start, start + 3).join(" ");
}

const _explanationMock: Record<string, string[]> = {
  fuzzy: [
...
const explanationContent: Record<string, string[]> = { fuzzy: _explanationMock.fuzzy, blank: _explanationMock.blank, internalized: _explanationMock.internalized };
function _origGetExplanation(status: string): string {
  const pool = explanationContent[status] || explanationContent.fuzzy;
