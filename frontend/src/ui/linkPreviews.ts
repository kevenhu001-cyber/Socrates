/* ui/linkPreviews.ts — URL reference cards shown beneath a user message.
 *
...
export function renderNoUrlHint(targetEl: HTMLElement | null): void {
  if (!targetEl) return;
...
export function renderLinkPreviews(
  targetEl: HTMLElement | null,
  urls: string[],
  results: (LinkPreviewResult | null)[] | null,
): void {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    const fetched: LinkPreviewResult | null = (results && results[i]) || null;
    const card = document.createElement('div');
