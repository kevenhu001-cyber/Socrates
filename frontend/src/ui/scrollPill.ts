// src/ui/scrollPill.ts — Phase C-2.2 extraction
...
export function showNewReplyPill(): void {
  const pill = document.getElementById("newReplyPill");
  if (pill) pill.classList.add("visible");
}

export function hideNewReplyPill(): void {
  const pill = document.getElementById("newReplyPill");
...
let wired = false;
export function wireScrollPill(): void {
  if (wired) return;
  wired = true;

  let debounceTmo: ReturnType<typeof setTimeout> | null = null;
