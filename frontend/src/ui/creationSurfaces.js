...
const state = { images: [], assistants: [], sites: [], page: null, loading: false, error: '', editor: null, busy: false, query: '', siteView: 'list' };
...
function header(name) {
  ...
  if (name === 'sites') return `<header class="creation-header creation-sites-header"><h1>${t(name)}</h1><input data-search type="search" placeholder="${t('searchSites')}" aria-label="${t('searchSites')}" value="${esc(state.query)}"></header><div class="creation-sites-view" role="group" aria-label="${t('listView')}"><button type="button" data-action="site-view" data-view="grid" aria-label="${t('gridView')}" aria-pressed="${state.siteView === 'grid'}">▦</button><button type="button" data-action="site-view" data-view="list" aria-label="${t('listView')}" aria-pressed="${state.siteView === 'list'}">☷</button></div><div class="creation-site-banner"><span class="creation-site-banner-icon" aria-hidden="true">▦</span><div><strong>${t('siteBanner')}</strong><small>${t('siteBannerBody')}</small></div><button class="creation-primary" data-action="new">${t('createSite')}</button></div>`;
...
export function renderCreationSurface(name) {
  ...
  state.page = name; state.query = ''; state.editor = null;
  load(name);
}
