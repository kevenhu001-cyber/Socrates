...
var activeTrigger: HTMLElement | null = null;

function label(key: string, fallback: string): string {
  try {
    var value = (window as any).t && (window as any).t(key);
...
function item(action: string, icon: string, title: string, description: string): string {
...
function _publishComposerTools(): void {
  try {
    var el = document.getElementById(MENU_ID);
    var bridge = (window as any).__socratesComposerToolsBridge;
...
/* React mode owns the menu's children. The legacy renderer becomes a
   ...
   never trips. */
function _reactOwnsMenu(): boolean {
  var el = document.getElementById(MENU_ID);
...
function position(el: HTMLElement, trigger: HTMLElement): void {
...
export function toggleComposerTools(trigger: HTMLElement, mode: string): void {
  var el = menu();
...
if (typeof document !== "undefined") {
  document.addEventListener("click", function (event: MouseEvent) {
    ...
    var action = (event.target as HTMLElement | null)?.closest?.("[data-action]") as HTMLElement | null;
    if (action && el.contains(action)) {
      var mode = activeTrigger && activeTrigger.dataset.composerMode;
      var kind = action.dataset.action;
      close();
      if (kind === "upload" && typeof (window as any).openAttachmentPicker === "function") (window as any).openAttachmentPicker(mode === "topic" ? "topicAttachInput" : "attachInput");
      if (kind === "write" && typeof (window as any).composeAction === "function") (window as any).composeAction();
      if (kind === "research" && typeof (window as any).researchAction === "function") (window as any).researchAction();
      if ((kind === "deepResearch" || kind === "exam") && typeof (window as any).toggleExtensionByKey === "function") (window as any).toggleExtensionByKey(kind);
      if (kind === "skills" && typeof (window as any).openPromptTemplatesModal === "function") (window as any).openPromptTemplatesModal();
      return;
    }
    if (!(event.target as HTMLElement | null)?.closest?.(".composer-tools-trigger")) close();
  });
  document.addEventListener("keydown", function (event: KeyboardEvent) { if (event.key === "Escape") close(); });
