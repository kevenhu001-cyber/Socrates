/* ui/findInSession.js — In-session find (Ctrl-F / Cmd-F).
 *
...
var _findMatches = []; /* <mark class="find-hl"> nodes, in document order */
var _findIndex = -1;   /* index of the active match, or -1 when none */
var _findQuery = "";   /* the current (trimmed) query */

function msgList() { return document.getElementById("msgList"); }
function findBar() { return document.getElementById("findBar"); }
function findInput() { return document.getElementById("findInput"); }
function findCount() { return document.getElementById("findCount"); }
...
function wrapMatchesInTextNode(node, needle) {
...
export function findNext() { if (_findMatches.length) setActive(_findIndex + 1); }
export function findPrev() { if (_findMatches.length) setActive(_findIndex - 1); }

export function onFindInput(q) { highlightMatches((q || "").trim()); }

export function onFindKey(ev) {
  if (ev.key === "Escape") {
    ev.preventDefault();
    closeFindInSession();
...
export function closeFindInSession() {
  var bar = findBar();
