...
export function renderSourcesCard(results: SourceResult[]): HTMLElement | null {
  ...
  wrap.addEventListener('click', function (ev: MouseEvent) {
    ...
    var copyBtn = t?.closest?.('.sources-copy') as HTMLElement | null;
    if (copyBtn) {
      ev.preventDefault();
      ev.stopPropagation();
      ...
      copyToClipboard(u).then(function () {
        ...
        try {
          if (typeof (window as any).showToast === "function") (window as any).showToast('Copy failed');
        } catch (_) {}
      });
      return;
    }
    var row = t?.closest?.('a.sources-row[data-sources-toggle]') as HTMLElement | null;
    if (row) {
...
function renderSourceRow(s: SourceResult, idx: number, opts?: SourceRowOpts): string {
  if (!s || !s.url) return "";
  opts = opts || {};
...
function renderSourceDetail(s: SourceResult, idx: number): string {
  var url = String(s.url);
  var html = '<div class="sources-detail" role="region" aria-label="Source ' + idx + ' details">';
  var engine = s.source || 'web';
  var engineLabel: Record<string, string> = { 'minimax-cli': 'Search', 'minimax': 'MiniMax', 'bing': 'Bing', 'searxng': 'SearXNG' };
  var label = engineLabel[engine] || engine;

  html += '<div class="sources-meta-row">';
  html += '<span class="sources-engine-pill" data-engine="' + esc(engine) + '"><span class="sources-engine-dot"></span>' + esc(label) + '</span>';
