...
/* Refresh every trigger without changing the existing composer pill: it
   continues to show the selected effort label, while the active model is
   available in the tooltip and accessible name. The bridge snapshot keeps
   aria-expanded / data-open in sync with the separate popover. */
export function syncEffortUI() {
  ...
  document.querySelectorAll(".effort-picker").forEach(function (picker) {
    ...
    var visibleLabel = picker.querySelector(".effort-label");
    if (visibleLabel) visibleLabel.textContent = effortLabel;
