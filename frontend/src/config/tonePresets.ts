// config/tonePresets.ts — AI tone/voice presets.
//
...
const TONE_KEY = "socrates-tone";
let _currentTone = "default";
...
function loadTonePreset(): string {
  try {
...
function setTonePreset(tone: string): void {
...
function getTonePreset(): string {
  return _currentTone;
}

/* Get the voice snippet for the current tone. */
function getToneVoice(): string {
  const preset = TONE_PRESETS[_currentTone];
...
function getAvailablePresets(): AvailablePreset[] {
...
function syncTonePresetUI(): void {
  const container = document.getElementById("tonePresetOptions");
  if (!container) return;
  const btns = container.querySelectorAll(".tone-preset-btn");
  btns.forEach(function (btn) {
    btn.classList.toggle("active", (btn as HTMLElement).dataset.tone === _currentTone);
  });
}

/* Render the tone preset radio buttons into the settings overlay. */
function renderTonePresets(): void {
  const container = document.getElementById("tonePresetOptions");
  if (!container) return;
  const presets = getAvailablePresets();
  const lang = (typeof (window as any)._currentLang === "string" && (window as any)._currentLang.indexOf("zh") === 0) ? "zh" : "en";
  container.innerHTML = presets.map(function (p) {
    const label = lang === "zh" ? (p.labelZh || p.label) : p.label;
    const desc = lang === "zh" ? (p.descriptionZh || p.description) : p.description;
...
if (typeof window !== "undefined") {
  (window as any).TONE_PRESETS = TONE_PRESETS;
  (window as any).loadTonePreset = loadTonePreset;
  (window as any).setTonePreset = setTonePreset;
  (window as any).getTonePreset = getTonePreset;
  (window as any).getToneVoice = getToneVoice;
  (window as any).getAvailablePresets = getAvailablePresets;
  (window as any).renderTonePresets = renderTonePresets;
  (window as any).syncTonePresetUI = syncTonePresetUI;
}
