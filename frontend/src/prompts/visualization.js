...
Common mistakes that cause a rejected spec: using \`points\`/\`values\` instead of \`data\`, \`expr\` instead of \`expression\`, \`title\` instead of \`name\` inside a series, \`source\`/\`target\` instead of \`from\`/\`to\` inside an edge, and omitting \`accessibilitySummary\`. Send no colors, fonts, sizes, CSS, or renderer options.
`;
