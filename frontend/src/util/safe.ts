...
export function sanitizeUrls(html: string): string {
  if (!html) return html;
  /* href on a, area, link; src on img, iframe, embed, source, track. */
  return html.replace(
    /\b(href|src|xlink:href|action|formaction)\s*=\s*("([^"]*)"|'([^']*)')/gi,
    (_m: string, attr: string, q: string, double?: string, sglt?: string): string => {
      const url = double !== undefined ? double : sglt!;
      const fixed = sanitizeUrl(url);
      if (fixed === url) return _m;
