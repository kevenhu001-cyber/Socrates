...
正式版本建议统一使用 `release-clients.yml`。

## 正式发布

正式发布由 `.github/workflows/release-clients.yml` 负责。它会先调用可复用的 `build-apk.yml`，执行前端、服务端、RN 检查，构建签名 Android APK/AAB，然后创建 GitHub Release 并附带总校验文件。
...
真实 Android 设备和 Google Play 审核仍属于发布后的验收环节；CI emulator smoke 不能替代这些审核。
